...
pkgs.writeShellScript "wpaperd-cycle-toggle" ''
  #!/usr/bin/env bash
  set -euo pipefail
  ...
  notify_off() {
    ${pkgs.libnotify}/bin/notify-send -a wpaperd -t 5000 -i wallpaper-symbolic 'Wallpaper Cycle Paused' 'Wallpaper cycle paused'
  }
  notify_on() {
    ${pkgs.libnotify}/bin/notify-send -a wpaperd -t 5000 -i wallpaper-symbolic 'Wallpaper Cycle Resumed' 'Wallpaper cycle resumed'
  }

  ironbar_do_off() {
    [ "$ironbar_enabled" = "true" ] || return 0
    ironbar_set_status 'wpaperd-cycle OFF' 'false'
  }
  ironbar_do_on() {
    [ "$ironbar_enabled" = "true" ] || return 0
    ironbar_set_status 'wpaperd-cycle ON' 'true'
  }
