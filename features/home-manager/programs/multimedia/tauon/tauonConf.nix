{config, ...}: {
  ...
  playback = {
    ...
    spotify-prefer-web = false; # Launch the web client rather than attempting to launch the desktop clientr
    spotify-allow-local = false; # Play Spotify audio through Tauon.
    back-restarts = true; # Pressing the back button restarts playing track on first press.
    resume-on-advance = false; # When paused, pressing back or next button starts playback.
    end-queue-stop = false; # Queue will always enable auto-stop on last track
    block-suspend = true; # Prevent system suspend during playback
    allow-video-formats = true; # Allow the import of MP4 and WEBM formats
  };

  HiDPI = {
    ui-scale = 2.0; # UI scale factor. Default is 1.0, try increase if using a HiDPI display.
  ...
  ui = {
    theme-name = "base16";
    transparent-style = 0; # 0=opaque(default), 1=accents
    ...
    side-panel-info-persists = true; # Show album art and metadata of last played track when stopped.
    side-panel-info-selected = true; # Show album art and metadata of selected track when stopped. (overides above setting)
  ...
  fonts = {
    ...
    font-main-standard = "${config.hm.theme.fonts.defaultMonospace}, Noto Sans CJK JP, Arial,"; # Suggested alternate: Liberation Sans
    font-main-medium = "${config.hm.theme.fonts.defaultMonospace}, Noto Sans CJK JP, Arial, Medium";
    font-main-bold = "${config.hm.theme.fonts.defaultMonospace}, Noto Sans CJK JP, Bold";
    font-main-condensed = "${config.hm.theme.fonts.defaultMonospace}, Extra-Condensed";
    font-main-condensed-bold = "${config.hm.theme.fonts.defaultMonospace}, Extra-Condensed Bold";
  };
